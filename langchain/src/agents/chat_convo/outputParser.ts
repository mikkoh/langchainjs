...
export class ChatConversationalAgentOutputParser extends AgentActionOutputParser {
  async parse(text: string) {
    ...
    if (!action_input) {
      throw new Error(`\`action_input\` could not be found in: ${trimmedText}`);
    }
  ...
  private findActionAndInput(text: string): { action: string | undefined; action_input: string | undefined } {
